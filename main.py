...
from schemas import GutachtenInput, GutachtenOutput, Gutachten, Theme, ThemeInput, ThemeOutput, GradeInput, GradeOutput, \
    Grade
...
def get_gutachten_by_id(ga_id: str, auth_payload=Depends(validate), session: Session = Depends(get_session)):
    query = select(Gutachten) \
        .where(Gutachten.id == ga_id) \
        .where(Gutachten.user_id == auth_payload.get("sub"))
...
def save_gutachten(ga: GutachtenInput, auth_payload=Depends(validate),
                   session: Session = Depends(get_session)) -> GutachtenOutput:
    print(ga)
...
def update_gutachten_by_id(ga_id: str, new_data: GutachtenInput, auth_payload=Depends(validate),
                           session: Session = Depends(get_session)) -> GutachtenOutput:
...
def update_theme_by_id(theme_id: str, new_data: ThemeInput, auth_payload=Depends(validate),
                       session: Session = Depends(get_session)) -> ThemeOutput:
